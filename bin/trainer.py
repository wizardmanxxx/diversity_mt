...
class Trainer(object):
    ...
    def __init__(self, model_par, train_data, vocab_data, optim, lossCompute, model, valid_data=None, tests_data=None,
        ...
        if wargs.fix_looking is True:
            ...
            wlog('randomly look {} samples from the whole training data'.format(self.n_look))
        ...
        self.n_eval = 1  # each epoch eval once ?
        ...
        self.ss_cur_prob = wargs.ss_prob_begin  # 选择采样衰减率，1 为 inverse sigmoid？
    ...
    def mt_eval(self, e_idx, e_bidx, n_steps):
        ...
        if wargs.save_one_model:
            ...
            model_file = '{}_type_{}_e{}_upd{}.pt'.format(wargs.model_prefix, wargs.reinfore_type, e_idx, n_steps)
        tc.save(state_dict, model_file)
        wlog('Saving temporary model in {}'.format(model_file))

        self.model_par.eval()
        self.model.eval()
        self.tor.trans_eval(self.valid_data, e_idx, e_bidx, n_steps, model_file, self.tests_data, self.writer)
        self.model_par.train()
        self.model.train()
        self.n_eval += 1
    ...
    def train(self):
        ...
        for e_idx in range(self.start_epoch, self.max_epochs + 1):
            ...
            bidx = 0
            cond = True if wargs.lr_update_way != 'invsqrt' else self.optim.learning_rate > wargs.min_lr  # what is cond ??
            while cond:
                ...
                if accum_batches == self.grad_accum_count:
                    ...
                    if current_steps % wargs.display_freq == 0:
                        ...
                        ''' 'Epo:e_idx/max_epochs |[b_counter/len(self.train_data) current_steps] |acc:{:5.2f}% |{:4.2f}/{:4.2f}=nll:{:4.2f} |bow:{:4.2f}'
                            ' |w-ppl:{:4.2f} |x(y)/s:{:>4}({:>4})/{}={}({}) |x(y)/sec:{}({}) |lr:{:7.6f}'
                            ' |{:4.2f}s/{:4.2f}m '''
                        wlog(
                            'Epo:{:>2}/{:>2} |[{:^5}/{} {:^5}] |acc:{:5.2f}% |{:4.2f}/{:4.2f}=nll:{:4.2f} |bow:{:4.2f}'
                            ' |w-ppl:{:4.2f} |x(y)/s:{:>4}({:>4})/{}={}({}) |x(y)/sec:{}({}) |lr:{:7.6f}'
                            ' |{:4.2f}s/{:4.2f}m'.format(  # b_counter?
                                e_idx, self.max_epochs, b_counter, len(self.train_data), current_steps,
                                (self.look_ok_ytoks / self.look_ytoks) * 100,  # acc
                                self.look_nll, self.look_ytoks, self.look_nll / self.look_ytoks,
                                # total_loss, total_words, normalize_loss
                                self.look_bow_loss / self.look_ytoks,  # bow
                                math.exp(self.look_nll / self.look_ytoks),  # perplex
                                self.look_xtoks, self.look_ytoks, self.look_sents,
                                # source words numbers, target words numbers, sentences numbers,
                                int(round(self.look_xtoks / self.look_sents)),  # average source sentences' length
                                int(round(self.look_ytoks / self.look_sents)),  # average target sentences' length
                                int(round(self.look_xtoks / ud)), int(round(self.look_ytoks / ud)),
                                self.optim.learning_rate, ud, (time.time() - train_start) / 60.)  # lr ? ?
                        )
                        if self.writer is not None:
                            self.writer.add_scalar('training/accuracy', (self.look_ok_ytoks / self.look_ytoks) * 100,
                                                   self.optim.n_current_steps)
                            self.writer.add_scalar('training/loss', self.look_nll / self.look_ytoks,
                                                   self.optim.n_current_steps)
                            self.writer.add_scalar('training/ppl', math.exp(self.look_nll / self.look_ytoks),
                                                   self.optim.n_current_steps)
                            self.writer.add_scalar('training/learning rate', self.optim.learning_rate,
                                                   self.optim.n_current_steps)
                        # if current_steps % (wargs.display_freq * 4) == 0:
                        #     for n, p in self.model.named_parameters():
                        #         self.writer.add_histogram('distribution/{}'.format(n), p.data)
            ...
            if wargs.epoch_eval is True:
                wlog('\nEnd epoch, e_batch:{}, n_steps:{}, {}-th validation ...'.format(e_bidx, current_steps,
                                                                                        self.n_eval))
