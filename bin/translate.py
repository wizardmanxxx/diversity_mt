...
if wargs.decoder_type in ('gru'):
    from searchs.nbs import *
elif wargs.decoder_type == 'att':
    from searchs.nbs_t2t import *


class Translator(object):

    def __init__(self, model, svcb_i2w=None, tvcb_i2w=None, search_mode=None, thresh=None, lm=None,
                 ngram=None, ptv=None, k=None, noise=None, print_att=False, gpu_ids=None, rl_future=None):

        self.svcb_i2w = svcb_i2w
        self.tvcb_i2w = tvcb_i2w
        self.lm = lm
        self.ptv = ptv  # what is ptv?
        self.noise = noise
        self.model = model
        self.ngram = ngram
        self.thresh = thresh
        ...
        self.\
            valid_k = 1
        # wlog('valid beam size is {}'.format(self.valid_k))
        self.valid_searcher = Nbs(model, self.tvcb_i2w, k=self.valid_k, noise=self.noise, print_att=print_att,
                                  gpu_ids=gpu_ids)
    ...
    def batch_trans_file(self, xs_inputs, batch_tst_data=None, valid=False):
        ...
        for bidx in range(n_batches):
            ...
            if False and attent_matrix != None:
                ...
                if isinstance(attent_matrix, list) and len(attent_matrix) == 0:
                    alnStr = ''
                else:
                    src_toks = [self.svcb_i2w[wid] for wid in x_filter]
                    # attent_matrix: (trgL, srcL) numpy
        ...
        if WN == 0:
            wlog('What ? No words generated when translating one file !!!')
        else:
    ...
    def token_batch_trans_file(self, xs_inputs, batch_tst_data=None, valid=False):
        _SN = xs_inputs.n_sent
        # point_every, number_every = int(math.ceil(n_batches/100)), int(math.ceil(n_batches/10))
        # point_every, number_every = int(math.ceil(_SN/100.)), int(math.ceil(_SN/10.))
        total_trans, total_losses, SN, WN = [], [], 0, 0
        total_aligns = [] if self.print_att is True else None
        fd_attent_matrixs, trgs = None, None
        # file = open('../back_translation.de', 'w')
        # file = io.open(, mode='a', encoding='utf-8')
        trans_start = time.time()
        bidx = 0
        while True:
            if xs_inputs.eos(): break
            # (idxs, tsrcs, lengths, src_mask) for test
            # (idxs, tsrcs, ttrgs_for_files, ttrg_bows_for_files, lengths, src_mask,
            # trg_mask_for_files, ttrg_bows_mask_for_files ) for valid and train
            batch = xs_inputs[bidx]
            ...
            # [[([2, 3], 8.789353370666504, 8.012884140014648, None)]]
            total_trans += list(top[0])  # [[2, 3, 4], [4, 5, 6]] for 2 sentences
            result = '\n'.join([idx2sent(one, self.tvcb_i2w)[-1] for one in top[0]])
            append_file('../back_translation_exp5.de', result)
            total_losses += list(top[1])  # original loss [2.3, 3.4] for 2 sentences
            # total_losses += list(top[2])    # norm loss
            SN += xs_nL.size(0)
            WN += sum([len(one_sent) for one_sent in list(top[0])])
    ...
    def trans_eval(self, valid_data, e_idx, e_bidx, n_steps, model_file, tests_data, writer):
        self.model.eval()

        wlog('\nTranslating validation dataset {}{}.{}'.format(wargs.val_tst_dir, wargs.val_prefix,
        ...
        valid_out = "{}_e{}_b{}_upd{}_k{}_type{}".format(outprefix, e_idx, e_bidx, n_steps, self.valid_k,
                                                         wargs.reinfore_type)
        ...
        if writer is not None:
            writer.add_scalar('valid/blue', bleu, n_steps)
            writer.add_scalar('valid/loss', wloss, n_steps)
