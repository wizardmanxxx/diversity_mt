...
def load_model(model_path):
    ...
    model_dict, e_idx, e_bidx, n_steps, optim = state_dict['model'], state_dict['epoch'], state_dict['batch'], \
                                                state_dict['steps'], state_dict['optim']
...
def format_time(time):
    ...
    rst = ''
    if time < 0.1:
        rst = '{:7.2f} ms'.format(time * 1000)
    elif time < 60:
        rst = '{:7.5f} sec'.format(time)
    elif time < 3600:
        rst = '{:6.4f} min'.format(time / 60.)
    else:
        rst = '{:6.4f} hr'.format(time / 3600.)
...
# beam search
def init_beam(beam, B=1, gpu_id=0, s0=None, cnt=50, score_0=0.0, loss_0=0.0, dyn_dec_tup=None, cp=False,
              transformer=False):
    del beam[:]
    ...
    if cp is True:
        beam[0] = [[(tc.tensor(loss_0).cuda(gpu_id), None, s0, _i, BOS * tc.ones(1).cuda(gpu_id).type(tc.int64), 0)] for
                   _i in range(B)]
        return
...
def back_tracking(beam, bidx, best_sample_endswith_eos, attent_probs=None):
    ...
    for i in reversed(range(0, endi)):  # [0, endi-1], with <bos> 0 and no <eos> endi==self.maxL
        ...
        if check is True:
            ...
            _, _, _, _, w, backptr = beam[i][bidx][bp]
        seq.append(w)
        bp = backptr
...
def sort_batches(srcs, trgs, slens, batch_size, k=1):
    ...
    if k == 0:
        ...
        wlog('done.')
    if len(final_srcs) == 0 and len(final_trgs) == 0:
        return srcs, trgs
    else:
