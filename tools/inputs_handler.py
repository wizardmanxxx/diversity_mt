...
def wrap_tst_data(src_data, src_vocab, char=False, is_sort=False):
    srcs, slens = [], []
    ...
    srcF.close()
    if is_sort:
        sort_index = sorted(range(len(slens)), key=lambda k: slens[k])
        sort_src = [srcs[i] for i in sort_index]
        return sort_src, slens, sort_index
    return srcs, slens
